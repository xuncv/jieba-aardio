...
CJiebaWordWithTag* CutWithTag(Jieba handle, const char* sentence, size_t len) {
    ...
    CJiebaWordWithTag* res = (CJiebaWordWithTag*)malloc(sizeof(CJiebaWordWithTag) * (tag_words.size() + 1));
    size_t offset = 0;
    for (size_t i = 0; i < tag_words.size(); i++) {
      res[i].word = sentence + offset;
      res[i].len = tag_words[i].first.size();
      memset(res[i].tag, '\0', 4);
      memcpy(res[i].tag,tag_words[i].second.data(), tag_words[i].second.size());
    ...
    res[tag_words.size()].word = NULL;
    res[tag_words.size()].len = 0;
    // res[tag_words.size()].tag = 0;
    return res;
}
